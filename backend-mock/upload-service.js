...
// MinIO Credentials configuration (defaults matching provided browser configurations)
...
// 3. Complete Chunked Upload (Assembles & uploads to MinIO)
app.post('/api/upload/complete', async (req, res) => {
  ...
  try {
    ...
    // Generate absolute URL for MinIO file access
    const minioUrl = `${minioUseSSL ? 'https' : 'http'}://${minioEndPoint}:${minioPort}/${minioBucket}/${objectName}`;
...
app.post('/api/upload/register-video', async (req, res) => {
  ...
  try {
    ...
    console.error("Failed to forward video metadata to n8n webhook:", err.message);
    res.status(500).json({
